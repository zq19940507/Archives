...
//    List<Wtb> findAll(@Param("starttime") String starttime, @Param("endtime") String endtime, @Param("location") String location);
    ...
    @Select("select location as location,zone as zone,dw as dw,community as community,\n" +
            ...
            " (location=#{location} or #{location} is null or #{location}='') and (zone=#{zone} or #{zone} is null or #{zone}='')and (community=#{community} or #{community} is null or #{community}='') group by id,user_amount order by wtb_amount desc;")
